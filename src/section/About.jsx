...
export default function About(){
  const stats = [
    ...
    {label: "Speciality", value: "Full Stack, GenAI"},
    {label: "Focus", value: "Performance & Backend"}
  ]
  const glows =[
      ...
      <div className='relative z-10 max-w-6xl w-full mx-auto px-6 md:px-10 lg:px-12 py-20 flex flex-col gap-12'>
        <motion.div className='flex flex-col md:flex-row items-center md:items-stretch gap-8' initial={{opacity:0, y:24}} whileInView={{opacity:1, y:0}} transition={{duration:0.6}} viewport={{once:true, amount:0.4}}>
          ...
          <div className='flex-1 flex flex-col justify-center text-center md:text-left'>
            ...
            <p className='mt-2 text-lg sm:text-xl text-white/90 font-semibold'>Full Stack Developer, GenAI Engineer</p>
            <p className='mt-4 text-gray-300 leading-relaxed text-base sm:text-lg max-w-2xl md:max-w-3xl'>
B.Tech Computer Science student specializing in Full-Stack Development and Generative AI. Built MERN-based web applications and Java desktop systems, including Jervis — an AI assistant integrating LLMs and intelligent automation. Strong foundation in Core Java, DSA, and backend architecture with hands-on experience in authentication, REST APIs, and responsive UI design.            </p>
            <div className='mt-6 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 max-w-xl'>
              {stats.map((item, i)=>(
                <motion.div key={i} className='rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-center' initial={{opacity:0, y:10}} whileInView={{opacity:1, y:0}} transition={{delay: 0.05 * 1, duration: 0.4}} viewport={{once: true, amount:0.3}}>
